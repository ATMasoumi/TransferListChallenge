...
final class LoadTransferFromRemoteUseCase: XCTestCase {
    ...
    func test_load_requestsDataFromURL() {
        ...
        let url = TransferEndpoint.get.url(baseURL: baseURL, page: page)
        let (sut, client) = makeSUT(baseURL: baseURL)

        sut.load(page: page) { _ in }
    ...
    func test_load_deliversErrorOnClientError() {
        ...
        expect(page: 1, sut, toCompleteWith: expectedResult) {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        ...
        samples.enumerated().forEach { index, code in
            expect(page: 1, sut, toCompleteWith: expectedResult, when: {
    ...
    func test_load_deliversNoItemsOn200HTTPResponseWithEmptyJSONList() {
        ...
        expect(page: 1, sut, toCompleteWith: .success([]), when: {
    ...
    func test_load_deliversItemsOn200HTTPResponseWithJSONItems() {
        ...
        expect(page: 1, sut, toCompleteWith: .success(items), when: {
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        var sut: RemoteTransferLoader? = RemoteTransferLoader(baseURL: url, client: client)
    ...
    private func makeSUT(baseURL: URL = URL(string: "https://a-url.com")!, file: StaticString = #file, line: UInt = #line) -> (sut: RemoteTransferLoader, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
        let sut = RemoteTransferLoader(baseURL: baseURL, client: client)
        trackForMemoryLeaks(sut, file: file, line: line)
        trackForMemoryLeaks(client, file: file, line: line)
        return (sut, client)
    }
    
    private func expect(page: Int, _ sut: RemoteTransferLoader, toCompleteWith expectedResult: RemoteTransferLoader.Result, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
        let exp = expectation(description: "Wait for load completion")
        
        sut.load(page: page) { receivedResult in
